...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Fetch employee details and UserID
        $sql_fetch_employee = "SELECT e.FirstName, e.LastName, u.EmailAddress, u.UserID
                               FROM employees e
                               JOIN useraccounts u ON e.EmployeeID = u.employeeID
                               WHERE e.EmployeeID = ?";
        $stmt_fetch = $conn->prepare($sql_fetch_employee);
        if ($stmt_fetch === false) {
            throw new Exception('Error preparing fetch statement: ' . $conn->error);
        }
        $stmt_fetch->bind_param('i', $employeeID);
        $stmt_fetch->execute();
        $stmt_fetch->bind_result($firstName, $lastName, $emailAddress, $userID);
        if (!$stmt_fetch->fetch()) {
        ...
        $activationLink = "https://epm-trucking-services-97f7106aa5cc.herokuapp.com/activate_account.php?token=$activationToken";
        ...
        $action = "Reset password for employee: " . $firstName . " " . $lastName;
